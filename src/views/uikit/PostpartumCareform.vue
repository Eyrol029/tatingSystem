...
const onAttendingStaffChange = () => {
  const staffID =
    form.value.attendingStaffID != null
      ? Number(form.value.attendingStaffID)
      : null;
...
async function loadExistingRecord() {
  ...
  try {
    ...
    if (records.length > 0) {
      ...
      form.value.healthFacilityName =
        latest.healthFacilityName || form.value.healthFacilityName;
      ...
      form.value.paraTerm =
        latest.paraTerm != null ? String(latest.paraTerm) : "";
      form.value.paraPreterm =
        latest.paraPreterm != null ? String(latest.paraPreterm) : "";
      form.value.paraAbortion =
        latest.paraAbortion != null ? String(latest.paraAbortion) : "";
      form.value.paraLiving =
        latest.paraLiving != null ? String(latest.paraLiving) : "";
      ...
      form.value.dateSigned =
        parseDateForForm(latest.dateSigned) || form.value.dateSigned;
      ...
      form.value.earlyBreastfeedingInitiation =
        !!latest.earlyBreastfeedingInitiation;
        ...
        latest.visits.forEach((v, i) => {
          if (i < form.value.visits.length) {
            form.value.visits[i] = {
              ...
              mms: !!v.mms,
              mmsQty: v.mmsQty != null ? String(v.mmsQty) : "",
              iron: !!v.iron,
              ironQty: v.ironQty != null ? String(v.ironQty) : "",
              folicAcid: !!v.folicAcid,
              folicAcidQty:
                v.folicAcidQty != null ? String(v.folicAcidQty) : "",
              calcium: !!v.calcium,
              calciumQty: v.calciumQty != null ? String(v.calciumQty) : "",
...
async function submitForm() {
  ...
  if (!serviceId) {
    submitStatus.value.error =
      "Service ID missing. Please go back and select a patient service.";
  ...
  const payload = {
    ...
    paraPreterm:
      form.value.paraPreterm !== "" ? Number(form.value.paraPreterm) : null,
    paraAbortion:
      form.value.paraAbortion !== "" ? Number(form.value.paraAbortion) : null,
    paraLiving:
      form.value.paraLiving !== "" ? Number(form.value.paraLiving) : null,
    ...
    visits: form.value.visits.map((v, i) => ({
      ...
      tetanusImmunization: v.tetanusImmunization,
      mms: v.mms,
      mmsQty: v.mmsQty !== "" ? Number(v.mmsQty) : null,
      iron: v.iron,
      ironQty: v.ironQty !== "" ? Number(v.ironQty) : null,
      folicAcid: v.folicAcid,
      folicAcidQty: v.folicAcidQty !== "" ? Number(v.folicAcidQty) : null,
      calcium: v.calcium,
      calciumQty: v.calciumQty !== "" ? Number(v.calciumQty) : null,
      counselingMentalHealth: v.counselingMentalHealth,
      counselingFamilyPlanning: v.counselingFamilyPlanning,
      counselingNutrition: v.counselingNutrition,
      lactationManagement: v.lactationManagement,
  ...
  try {
    if (existingRecordId.value) {
      await axios.put(BASE, {
        ...payload,
        postpartumRecordID: existingRecordId.value,
      });
      submitStatus.value.success =
        "✅ Postpartum Care record updated successfully!";
    } else {
      const res = await axios.post(BASE, payload);
      existingRecordId.value = res.data.postpartumRecordID;
      submitStatus.value.success =
        "✅ Postpartum Care record saved successfully!";
    }
    ...
    if (returnToBilling && admissionServiceId) {
      if (isVisitsCompleted.value) {
        submitStatus.value.success =
          "✅ Postpartum Care record saved! 3rd & 4th visits completed — proceeding to Billing...";
        setTimeout(() => {
          router.push({
        ...
        submitStatus.value.success =
          "✅ Postpartum Care record saved successfully! (Note: 3rd and 4th visits must be completed before proceeding to Billing/Payment).";
      }
    }
  } catch (error) {
    const msg =
...
<template>
  ...
  <div class="no-print max-w-screen mx-auto px-6 pt-6 flex items-center gap-3">
    ...
    <span
      v-if="isReadOnly"
      class="ml-2 text-xs font-semibold text-indigo-700 bg-indigo-100 px-3 py-1.5 rounded-full"
    >
      👁️ View Only
    </span>
  </div>
  ...
  >
      ...
      <div class="flex items-center justify-center gap-3 mb-2">
        ...
        <div class="text-left">
          <p class="text-xs text-gray-500 leading-tight">
            Tating's Birthing Home and Family Planning Clinic
          </p>
          <p class="text-xs text-gray-400 leading-tight">
            Purok 26, Poblacion, Manticao, Misamis Oriental
          </p>
          <h1
            class="text-lg font-bold tracking-wide uppercase leading-tight mt-1"
          >
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label class="field-label"
            >PhilHealth Accreditation Number (PAN)</label
          >
          <input
            type="text"
        ...
        <div>
          <label class="field-label"
            >PhilHealth Identification Number (PIN)</label
          >
          <input
            type="text"
            v-model="form.pin"
      ...
      <div class="mb-6">
        <label class="field-label"
          >6. Brief History of Present Illness / OB History</label
        >
        <textarea
          v-model="form.briefHistory"
          rows="3"
          class="w-full border border-gray-300 rounded p-2 text-sm"
          placeholder="Enter brief history or OB history..."
        ></textarea>
      </div>
      ...
      <p class="text-xs text-gray-500 mb-2 font-semibold">
        7. General Survey (Pertinent Findings Per System)
      </p>
        ...
        <table class="w-full border-collapse text-xs visit-table">
          <thead>
            <tr class="bg-gray-100">
              ...
              <th class="td" v-for="(v, i) in form.visits" :key="'h' + i">
                {{ i + 1 }}{{ ["st", "nd", "rd", "th"][i] }}
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr>
              ...
              <td
                ...
                v-for="(v, i) in form.visits"
                :key="'tet' + i"
              >
                <input
                  type="checkbox"
                  v-model="v.tetanusImmunization"
                  class="w-4 h-4"
                />
              </td>
            </tr>
            ...
            <tr>
              ...
              >
                <input
                  type="checkbox"
                  v-model="v.counselingMentalHealth"
                  class="w-4 h-4"
                />
              </td>
            </tr>
            <tr>
              ...
              >
                <input
                  type="checkbox"
                  v-model="v.counselingFamilyPlanning"
                  class="w-4 h-4"
                />
              </td>
            </tr>
      ...
      <div class="mb-4 bg-purple-50/60 border border-purple-200 rounded-lg p-4">
        <h4
          class="font-bold text-sm text-purple-900 mb-3 flex items-center gap-2"
        >
          <span></span> 1. Immediate Newborn Care (EINC Checklist)
        </h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label
            class="flex items-center gap-2 bg-white p-2.5 rounded border border-purple-100 hover:bg-purple-50 cursor-pointer transition text-xs font-medium text-gray-800"
          >
            <input
              type="checkbox"
              v-model="form.immediateDrying"
          ...
          >
            <input
              type="checkbox"
              v-model="form.skinToSkinContact"
              class="w-4 h-4 text-purple-600 rounded"
            />
          ...
          >
            <input
              type="checkbox"
              v-model="form.properCordClamping"
              class="w-4 h-4 text-purple-600 rounded"
            />
          ...
          >
            <input
              type="checkbox"
              v-model="form.earlyBreastfeedingInitiation"
              class="w-4 h-4 text-purple-600 rounded"
            />
      ...
      <div class="mb-4 bg-teal-50/60 border border-teal-200 rounded-lg p-4">
        <h4
          class="font-bold text-sm text-teal-900 mb-3 flex items-center gap-2"
        >
          <span></span> 2. Routine Preventive Care
        </h4>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          <label
            class="flex items-center gap-2 bg-white p-2.5 rounded border border-teal-100 hover:bg-teal-50 cursor-pointer transition text-xs font-medium text-gray-800"
          >
            <input
              type="checkbox"
              v-model="form.eyeProphylaxis"
          ...
          >
            <input
              type="checkbox"
              v-model="form.vitaminKAdministered"
              class="w-4 h-4 text-teal-600 rounded"
            />
          ...
          >
            <input
              type="checkbox"
              v-model="form.hepatitisBVaccine"
              class="w-4 h-4 text-teal-600 rounded"
            />
      ...
      <div class="mb-4 bg-blue-50/60 border border-blue-200 rounded-lg p-4">
        <h4
          class="font-bold text-sm text-blue-900 mb-3 flex items-center gap-2"
        >
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. 3,200 g / 3.2 kg"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              type="text"
              v-model="form.chestCircumference"
              class="input bg-white"
          ...
          <div>
            ...
            <input
              type="text"
              v-model="form.gestationalAgeWeeks"
              class="input bg-white"
      ...
      <div class="mb-4 bg-amber-50/60 border border-amber-200 rounded-lg p-4">
        <h4
          class="font-bold text-sm text-amber-900 mb-3 flex items-center gap-2"
        >
          <span></span> 4. Screening Tests
        </h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="field-label font-semibold text-gray-800"
              >Newborn Screening (NBS) — Filter Paper Blood Spot</label
            >
            <input
              type="text"
              v-model="form.newbornScreening"
              class="input bg-white"
              placeholder="e.g. Done / Blood spot collected / Referred"
            />
          </div>
          <div>
            <label class="field-label font-semibold text-gray-800"
              >Newborn Hearing Screening Test (NHST)</label
            >
            <input
              type="text"
              v-model="form.newbornHearingScreening"
              class="input bg-white"
              placeholder="e.g. Done (Pass) / Referred"
            />
          </div>
        </div>
      </div>
      ...
      <div class="mb-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
        <label
          class="field-label text-gray-800 font-bold text-sm mb-2 flex items-center gap-2"
        >
          <span></span> 5. Newborn Care Notes / Clinical Observations
        </label>
        <textarea
          v-model="form.newbornCareNotes"
          rows="3"
          class="w-full border border-gray-300 rounded p-2 text-sm bg-white focus:ring-2 focus:ring-purple-500 focus:outline-none"
          placeholder="Enter newborn care notes, clinical observations, feeding status, instructions given to parents..."
        ></textarea>
      </div>
      ...
      <div class="border-l-4 border-indigo-500 bg-indigo-50 p-4 mb-6 rounded-r">
        <p class="font-bold mb-2">
          10. Certification of Attending Physician/Midwife:
        </p>
      ...
      <div class="grid grid-cols-2 gap-16 mt-10 mb-6">
        <div class="text-center flex flex-col justify-end min-h-[4rem]">
          <div class="border-t border-gray-800 pt-1">
            <p class="text-xs font-semibold">
              {{ form.attendingSignatureName || "Signature" }}
            </p>
        ...
        <div class="text-center flex flex-col justify-end min-h-[4rem]">
          <input
            type="text"
            v-model="form.patientConformeName"
            placeholder="Patient's printed name"
            class="input-line text-center w-full mb-1"
          />
          <div class="border-t border-gray-800 pt-1">
            <p class="text-xs font-semibold">
              11. Patient's Conforme — Signature / Printed Name
            </p>
          </div>
        </div>
      </div>
      ...
      >
        {{
          submitStatus.loading
            ? "Saving..."
            : existingRecordId
              ? "UPDATE POSTPARTUM CARE RECORD"
              : "SAVE POSTPARTUM CARE RECORD"
        }}
      </button>
    </div>
  </div>
